...
public class Draw2_Solution extends JFrame {
	...
	EventQueue.invokeLater(new Runnable() {
	    public void run() {
		try {
		    Draw2_Solution frame = new Draw2_Solution();
	...
	btnZeichnen.addActionListener(new ActionListener() {
		...
		try {
			...
			if (chckbxGefuellt.isSelected()) {
			    panelZeichenflaeche.getGraphics().fillRect(x1, y1,
				    x2, y2);
			} else {
			    panelZeichenflaeche.getGraphics().drawRect(x1, y1,
				    x2, y2);
			}
			break;
		    case 'K':
			if (chckbxGefuellt.isSelected()) {
			    panelZeichenflaeche.getGraphics().fillOval(x1, y1,
				    x2, x2);
			} else {
			    panelZeichenflaeche.getGraphics().drawOval(x1, y1,
				    x2, x2);
			}
			break;
		    case 'O':
			...
			break;
		    case 'L':
			panelZeichenflaeche.getGraphics().drawLine(x1, y1, x2,
				y2);
			break;
		    }
		} catch (Exception ex) {
